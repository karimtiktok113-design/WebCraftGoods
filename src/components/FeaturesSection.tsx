import React from 'react';
import { motion } from 'motion/react';
import {
  Zap,
  ShieldCheck,
  RefreshCw,
  Sparkles,
  Layers,
  Headphones,
  Code2,
  Cpu,
  Palette,
  CheckCircle,
  Smartphone,
  Globe,
  HardDrive,
  BarChart3,
  Share2,
  Download,
  Upload,
  RotateCcw,
  LucideIcon,
} from 'lucide-react';
import { useData } from '../context/DataContext';

// Map icon string names or emojis to Lucide icons
export const ICON_MAP: Record<string, LucideIcon> = {
  Zap,
  '⚡': Zap,
  Palette,
  '🎨': Palette,
  Smartphone,
  '📱': Smartphone,
  Globe,
  '🌐': Globe,
  HardDrive,
  '💾': HardDrive,
  RefreshCw,
  '🔄': RefreshCw,
  BarChart3,
  '📊': BarChart3,
  Share2,
  '📤': Share2,
  Download,
  Upload,
  RotateCcw,
  ShieldCheck,
  Sparkles,
  Layers,
  Headphones,
  Code2,
  Cpu,
  CheckCircle,
};

export const FeaturesSection: React.FC = () => {
  const { features } = useData();

  return (
    <section id="features" className="py-16 sm:py-24 bg-slate-900/40 relative border-t border-slate-800/80 overflow-hidden w-full">
      {/* Subtle ambient lighting */}
      <div className="absolute top-1/4 -right-20 w-80 h-80 bg-amber-500/5 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-1/4 -left-20 w-80 h-80 bg-indigo-500/5 rounded-full blur-3xl pointer-events-none" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        {/* Section Heading */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}
          className="text-center max-w-3xl mx-auto mb-10 sm:mb-16"
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900 border border-slate-800 text-amber-400 text-xs font-semibold mb-4 shadow-sm shadow-amber-500/5">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Built for Modern Productivity</span>
          </div>
          <h2 className="text-2xl sm:text-4xl font-extrabold text-white font-heading tracking-tight mb-3 sm:mb-4">
            Powerful Features & Thoughtful Design
          </h2>
          <p className="text-sm sm:text-base text-slate-400 font-light leading-relaxed">
            Powerful features, thoughtful design, and practical functionality come together to create digital tools you'll actually enjoy using.
          </p>
        </motion.div>

        {/* Feature Cards Grid - 8 items in 4 columns on lg, 2 columns on sm/md */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
          {features.map((feature, index) => {
            const IconComponent = ICON_MAP[feature.icon] || Sparkles;
            return (
              <motion.div
                key={feature.id}
                id={`feature-card-${feature.id}`}
                initial={{ opacity: 0, y: 16 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.4, delay: Math.min(index * 0.05, 0.25), ease: [0.22, 1, 0.36, 1] }}
                whileHover={{ y: -6, transition: { duration: 0.22 } }}
                className="group p-5 sm:p-6 rounded-2xl bg-slate-900/60 hover:bg-slate-900 border border-slate-800/80 hover:border-amber-500/40 transition-[background-color,border-color,box-shadow] duration-300 transform-gpu shadow-md hover:shadow-xl hover:shadow-amber-500/10 cursor-default flex flex-col justify-between"
              >
                <div>
                  <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 flex items-center justify-center mb-5 sm:mb-6 group-hover:scale-110 group-hover:bg-amber-500/20 group-hover:border-amber-500/40 transition-all duration-300 shadow-sm shadow-amber-500/10">
                    <IconComponent className="w-5 h-5 sm:w-6 sm:h-6" />
                  </div>
                  <h3 className="text-base sm:text-lg font-bold text-white font-heading tracking-tight mb-2 sm:mb-3 group-hover:text-amber-300 transition-colors">
                    {feature.heading}
                  </h3>
                  <p className="text-xs sm:text-sm text-slate-400 leading-relaxed font-light">
                    {feature.description}
                  </p>
                </div>
              </motion.div>
            );
          })}
        </div>
      </div>
    </section>
  );
};
